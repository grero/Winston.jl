...
@compat set(self::CairoRenderer, key::AbstractString, value) = set(self, symbol(key), value)

function get(self::CairoRenderer, parameter::Symbol, notfound=nothing)
    return get(self.state, parameter, notfound)
end
@compat get(self::CairoRenderer, parameter::AbstractString, notfound=nothing) = get(self, symbol(parameter), notfound)
...
@compat function layout_text(self::CairoRenderer, str::AbstractString)
    set_latex(self.ctx, str, get(self,:fontsize))
end

@compat function textdraw(self::CairoRenderer, x::Real, y::Real, str::AbstractString; kwargs...)
